...
const PROJECT_HEADERS = ['ProjectID', 'ProjectName', 'ClientName', 'ClientCode', 'ContactName', 'CustomerAddress', 'CustomerPhone', 'BriefMoq', 'BriefBudget', 'BriefSize', 'BriefTexture', 'BriefScent', 'BriefColor', 'BriefExtracts', 'BriefAvoid', 'BriefConcept', 'Brief', 'DateReceived', 'DueDate', 'Stage', 'LastUpdated', 'Owner'];
...
const USER_HEADERS = ['Username', 'PasswordHash', 'Role', 'LineUserID', 'ClientCode', 'CustomerPhone', 'CreatedAt'];
...
function doPost(e) {
  ...
  if (action === 'linelogin') {
    const user = findLineUser_(body.LineUserID, body.LineAccessToken);
    if (!user) return json_({ error: 'unregistered_line_user' });
  ...
  const user = findRequestUser_(body);
  if (!user) return json_({ error: 'unauthorized' });
...
function canCreateRole_(user, role) {
  ...
  const targetRole = role_({ Role: role });
  if (currentRole === 'RD') return targetRole === 'CUSTOMER';
  if (currentRole === 'MM') return ['RD', 'CUSTOMER'].indexOf(targetRole) !== -1;
  if (currentRole === 'MD') return ['CS', 'RD', 'MM', 'MD', 'CUSTOMER'].indexOf(targetRole) !== -1;
  return false;
}
...
function role_(user) {
  ...
  return { client: 'CUSTOMER', customer: 'CUSTOMER', staff: 'RD', admin: 'MD', rd: 'RD', manager: 'MM', md: 'MD', cs: 'CS', mm: 'MM' }[role] || String((user && user.Role) || '').toUpperCase();
}
...
function findRequestUser_(body) {
  if (body.LineUserID || body.LineAccessToken) return findLineUser_(body.LineUserID, body.LineAccessToken);
  return findUser_(body.AuthUsername, body.AuthPasswordHash);
}
function findLineUser_(lineUserId, accessToken) {
  if (!lineUserId || !accessToken || verifyLineUserId_(accessToken) !== lineUserId) return null;
  return listUsers_().find(user => user.LineUserID === lineUserId) || null;
}
function verifyLineUserId_(accessToken) {
  try {
...
function publicUser_(user) {
  return { Username: user.Username, PasswordHash: user.PasswordHash, Role: role_(user), LineUserID: user.LineUserID || '', ClientCode: user.ClientCode || '', CustomerPhone: user.CustomerPhone || '' };
}
...
function groupByProject_(rows, sortField) {
  return rows.sort((a, b) => String(b[sortField]).localeCompare(String(a[sortField]))).reduce((out, row) => {
    out[row.ProjectID] = out[row.ProjectID] || [];
